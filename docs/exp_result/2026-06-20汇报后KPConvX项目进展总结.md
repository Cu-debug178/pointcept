...
当前汇报中存在三条容易被混淆的结果。第一条是**自训练权重**，后两条才是**同一份 Apple 官方权重**的双协议对照。

| 编号 | 推荐名称 | 权重来源 | 模型/测试入口 | 实际测试协议 | mIoU | 该结果回答的问题 |
| --- | --- | --- | --- | --- | ---: | --- |
| A | 自训练 scale04 identity | 自训练 `kpconvx-scale04-baseline` epoch 200 | Pointcept fixed evaluator | 0.04m、identity 单视角、fragment 概率回填 | **69.3926%** | 当前自训练模型在固定单视角协议下有多好 |
| B | 官方权重 Standalone 10-vote | Apple `S3DIS_KPConvX-L` epoch 300 | 官方 `test_S3DIS.py` | 整房间、10 次随机 vote、完整点云重投影 | **73.47%** | 是否复现 Apple 公开的 `73.5%` |
| C | 官方权重 Pointcept 4-rotation | 与 B 完全相同的 Apple tensor，仅增加 `backbone.` 前缀 | Pointcept 专用官方权重配置 | 0.04m、4 个固定旋转、fragment 概率回填 | **68.07%** | 同一官方权重换成 Pointcept 推理协议后会得到什么结果 |

最关键的口径是：

```text
A 不是 B/C 的同权重测试。
B 和 C 才是同一官方权重的推理协议对照。
A 当前没有使用 TTA13，也没有使用 official-like vote10。
C 使用了 4 个固定旋转，但不是 TTA13，也不是 official-like vote10。
```
...
-> TestSphereCrop，每个 fragment 最多 60000 点
-> identity 单视角推理
-> fragment softmax 按原始 index 回填并累加
-> 完整 Area5 混淆矩阵
-> mIoU 69.3926%
```

### 10.5 同一官方权重的 Standalone 与 Pointcept 全流程对照

2026-07-16 使用 Apple 发布的 `S3DIS_KPConvX-L` epoch-300 权重完成 Area5 全部 68 个房间的两次测试：

| 对比环节 | B：Apple Standalone 官方协议 | C：Pointcept 对照协议 |
| --- | --- | --- |
| 权重 | 官方 `current_chkp.tar` | 同一组 864 个 tensor，仅键增加 `backbone.` 前缀 |
| 输入特征 | 5 维 `[1, R, G, B, 原始 z]` | 同样显式构造 5 维 `[1, R, G, B, 原始 z]` |
| 初始尺度 | 0.04m initial subsampling | 0.04m `GridSample(mode="test")` |
| 场景组织 | `in_radius=100m`，等效完整房间一次输入 | 大房间拆成最多 60000 点的 fragments |
| sampler | regular sampler，batch size/limit 都为 1 | Pointcept test voxel partitions，fragment batch 8 |
| 中心化 | XY 均值中心化 + floor Z | bbox 中心化 + floor Z，fragment 后再做 XY CenterShift |
| 测试增强 | 每个 vote 随机垂直旋转、`0.99-1.01` 缩放、X 轴 50% 翻转 | 固定 `0/90/180/270` 度 4 个旋转 |
| 测试次数 | 10 votes | 4 fixed views |
| 概率融合 | 跨 vote 持续累计，`test_momentum=0.95` | fragment softmax 按原始 `index` 累加 |
| 全分辨率恢复 | 从预下采样点最近邻 projection 回完整原始点云 | fragment 预测直接按 index 回填原始场景点 |
| 最终指标 | **73.47 / 78.51 / 91.74%** | **68.07 / 72.99 / 89.91%** |

其中最终指标顺序为 `mIoU / mAcc / OA`。Standalone 的 `73.47%` 四舍五入后就是论文/官方公开的 `73.5%`，因此官方结果已经成功复现。
...
所以 B 与 C 相差 `5.40` 个百分点不是 checkpoint 转换损失，而是整房间预下采样/回投、中心化、邻域构建、测试增强次数和概率聚合方式共同造成。缺少官方 10-vote 是其中一个因素，但当前没有单变量实验，不能把全部 `5.40` 都归因于 TTA/voting。

需要注意，Apple Standalone 日志对数值标签 7/8/9/10 的显示名称与当前 Pointcept 名称顺序不同；总体混淆矩阵和 mIoU 不受影响，单类比较必须按数值标签核对。

### 10.6 自训练 scale04 与 Apple 官方模型配置差异

结果 A 与 B/C 不仅测试协议不同，模型和训练过程也不同：

| 配置项 | A：自训练 scale04 plain KPConvX | B/C：Apple 官方 KPConvX-L |
| --- | --- | --- |
| 权重来源 | Pointcept 内自行训练 | Apple 发布的 epoch-300 权重 |
| 输入维度 | 9 维 `coord + color + normal` | 5 维 `[1, RGB, z]` |
| shell sizes | `(1, 14, 28)`，43 kernel points | `(1, 14, 42)`，57 kernel points |
| `kp_radius / kp_sigma` | `2.1 / 2.1` | `2.1 / 2.1` |
| influence | `constant` | `linear` |
| `share_kp` | `False` | `True` |
| neighbor limits | `(12, 16, 20, 20, 20)` | `(12, 16, 20, 20, 20)` |
| 主训练管线 | Pointcept，batch 3，无梯度累积，CE + Lovasz | Standalone，batch 4、梯度累积 6、官方 sampler/scheduler |
| 当前测试 | identity 单视角 fragment | B 为官方 10-vote；C 为 Pointcept 4-rotation fragment |

因此不能据此宣称 A 的 `69.39%` 高于 C 的 `68.07%`，也不能假设给 A 加 10-vote 就会达到 B 的 `73.47%`。C 的用途只是隔离同一官方权重在两套推理协议下的变化，不是新的模型排行榜基线。
...
1. **结果归属页**：只放 A/B/C 三条结果，强调 A 是自训练、B/C 是同一官方权重。
2. **协议差异页**：只比较 B 与 C 的整房间/fragment、10 votes/4 rotations、EMA/index 回填和 projection。
3. **模型差异页**：比较 A 与 B/C 的 9D/5D、43/57 kernel points、constant/linear、share-kp 和训练预算，说明 A/B 不能直接排名。
...
| **A：scale04 plain KPConvX** | **0.04m fixed identity epoch 200** | **0.693926** | **自训练权重；当前可信主基线** |
...
| B：Apple 官方 KPConvX-L | Standalone 官方 10-vote | **73.47%** | 官方权重；成功复现官方 `73.5%` |
| C：同一 Apple 官方权重 | Pointcept 4-rotation fragment | 68.07% | 官方权重；与 B 的协议差异为 `-5.40` 个百分点 |
