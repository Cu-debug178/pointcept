...
#
#   KPConvX project: object_classification.py
#       > Object classification dataset class
#
...
class ObjClassifDataset(Dataset):
    ...
    def __getitem__(self, idx_list):
        ...
        if self.set == 'training':
            rsmix_beta = self.cfg.augment_train.rsmix_beta
            rsmix_prob = self.cfg.augment_train.rsmix_prob
            rsmix_nsample = self.cfg.augment_train.rsmix_nsample
        else:
            rsmix_beta = self.cfg.augment_test.rsmix_beta
            rsmix_prob = self.cfg.augment_test.rsmix_prob
            rsmix_nsample = self.cfg.augment_test.rsmix_nsample

        if rsmix_beta > 0: 
  
            if np.random.rand(1) < rsmix_prob:

                # Variable batch clouds not supported
                if np.any([tp_list[0].shape[0] != tp.shape[0] for tp in tp_list]):
                    raise ValueError('All clouds must have the same number of points for RSMix augmentation.')

                # Get numpy arrays
                new_data = [tp.numpy() for tp in tp_list]
                new_data = np.stack(new_data, axis=0)  # (B, N, 3)
                new_label = np.array(tl_list, dtype=np.int64)  # integers (B)

                # Add features
                feat_np = np.stack([tf.numpy() for tf in tf_list], axis=0)
                new_data = np.concatenate([new_data, feat_np], axis=-1)

                new_data, lam, new_label, label_b = rsmix(new_data, 
                                                            new_label, 
                                                            beta=rsmix_beta, 
                                                            n_sample=rsmix_nsample)

                # Update lists
                tp_list = [torch.from_numpy(nd) for nd in new_data[:, :, :3].astype(np.float32)]
                tf_list = [torch.from_numpy(nd) for nd in new_data[:, :, 3:].astype(np.float32)]
                tl_list = new_label.tolist()
        ...
        # RSmix variables
        labels_b = torch.LongTensor(label_b)
