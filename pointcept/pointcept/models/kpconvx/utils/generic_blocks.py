...
#
#   KPConvX project: generic_blocks.py
#       > Define generic network blocks
#
...
class MaxPoolBlock(nn.Module):
    ...
    def forward(self, x, neighb_inds, idx_ptr=None):

        # Grid pool mode from pointcept
        if idx_ptr is not None:
            # sorted_cluster_indices = neighb_inds
            x = segment_csr(x[neighb_inds], idx_ptr, reduce="max")
            return x
        
        # Old grid pool with neighbor gathering
        else:
...
class SmoothCrossEntropyLoss(torch.nn.Module):
    def __init__(self, label_smoothing=0.2, 
        ...
        if ignore_index is not None:
            ...
            self.reducing_list = torch.cat([reducing_list[:ignore_index], inserted_value,
                                            reducing_list[ignore_index:]], 0)
