...
CHECKPOINT_KIND="epoch_150"
MANIFEST="$PROJECT/configs/s3dis/eval/v17_scale04_epoch150_concurrent_preflight.json"
...
RUN_LOG="$LOG_ROOT/v17_scale04_concurrent_full_${TIMESTAMP}.log"
LAUNCH_STATE="$LOG_ROOT/v17_scale04_concurrent_full_${TIMESTAMP}.state"
TELEMETRY="$TELEMETRY_ROOT/v17_scale04_concurrent_full_${TIMESTAMP}_gpu.csv"
LATEST_LOG="$LOG_ROOT/latest_v17_scale04_concurrent_full.log"
LATEST_STATE="$LOG_ROOT/latest_v17_scale04_concurrent_full.state"
LOCK_FILE="$PROJECT/exp/fixed_protocol/v17_scale04_concurrent_full.lock"
CHECKPOINT="$PROJECT/exp/s3dis/$RUN_ID/model/$CHECKPOINT_KIND.pth"
...
if [ ! -f "$CHECKPOINT" ]; then
  echo "Missing immutable checkpoint: $CHECKPOINT" >&2
  exit 3
fi

exec 9> "$LOCK_FILE"
if ! flock -n 9; then
  echo "Another concurrent full evaluation holds $LOCK_FILE" >&2
  exit 5
fi

echo "$RUN_LOG"
ln -sfn "$RUN_LOG" "$LATEST_LOG"
ln -sfn "$LAUNCH_STATE" "$LATEST_STATE"
echo "timestamp,memory_used_mib,memory_free_mib,gpu_util_percent,power_draw_w,training_alive" > "$TELEMETRY"

# The evaluation gets lower CPU and disk priority so training remains the primary job.
setsid nice -n 10 ionice -c 3 env \
  ...
  "$PYTHON_BIN" -u tools/eval_s3dis_fixed_protocol.py \
    ...
    --num-worker-test 2 \
    --checkpoint-kinds "$CHECKPOINT_KIND" \
    --run-ids "$RUN_ID" \
    --cuda-memory-fraction "$MEMORY_FRACTION" \
    > "$RUN_LOG" 2>&1 &
EVAL_PID=$!

cat > "$LAUNCH_STATE" <<EOF
wrapper_pid=$$
evaluation_pid=$EVAL_PID
...
cleanup() {
  ...
  sed -i "s/^status=.*/status=finished/" "$LAUNCH_STATE"
  echo "exit_code=$EXIT_CODE" >> "$LAUNCH_STATE"
}
trap cleanup EXIT
trap 'exit 143' TERM
trap 'exit 130' INT

echo "Concurrent full evaluation PID: $EVAL_PID"
