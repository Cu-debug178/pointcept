...
OUTPUT_ROOT="$PROJECT/exp/fixed_protocol/results/v17-scale04-concurrent-preflight"
LOG_ROOT="$PROJECT/exp/fixed_protocol/logs"
TELEMETRY_ROOT="$PROJECT/exp/fixed_protocol/telemetry"
TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
RUN_LOG="$LOG_ROOT/v17_scale04_concurrent_preflight_${TIMESTAMP}.log"
TELEMETRY="$TELEMETRY_ROOT/v17_scale04_concurrent_preflight_${TIMESTAMP}_gpu.csv"
LATEST_LOG="$LOG_ROOT/latest_v17_scale04_concurrent_preflight.log"
LOCK_FILE="$PROJECT/exp/fixed_protocol/v17_scale04_concurrent_preflight.lock"
...
if [ ! -f "$PROJECT/exp/s3dis/kpconvx-v17-scale04_20260714_1553/model/epoch_150.pth" ]; then
  echo "Missing immutable checkpoint epoch_150.pth" >&2
  exit 3
fi

exec 9> "$LOCK_FILE"
if ! flock -n 9; then
  echo "Another concurrent preflight holds $LOCK_FILE" >&2
  exit 5
fi
...
echo "Concurrent evaluation PID: $EVAL_PID"
...
while kill -0 "$EVAL_PID" 2>/dev/null; do
  ...
  if [ "$TRAINING_ALIVE" -ne 1 ]; then
    echo "Training PID disappeared; stopping evaluation only." >> "$RUN_LOG"
    terminate_eval
    exit 6
  fi
